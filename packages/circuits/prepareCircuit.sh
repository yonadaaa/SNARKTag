...
snarkjs groth16 setup "${circuitName}.r1cs" ../pot/pot20_final.ptau circuit_0000.zkey
...
sed -i -e "s/uint\[2\] memory a/uint[8] memory proofData/g" $fileName
sed -i -e "/memory b,/d" $fileName
sed -i -e "/memory c,/d" $fileName
sed -i -e "s/a\[0\], a\[1\]/proofData[0], proofData[1]/g" $fileName
sed -i -e "s/\[b\[0\]\[0\], b\[0\]\[1\]\], \[b\[1\]\[0\], b\[1\]\[1\]\]/[proofData[2], proofData[3]], [proofData[4], proofData[5]]/g" $fileName
sed -i -e "s/c\[0\], c\[1\]/proofData[6], proofData[7]/g" $fileName

# Create folder in static/circuits/ for the circuit if it doesn't already exist, and
# copy over the necessary files
circuitDir="../../client/src/public/circuits/$circuitName"
mkdir -p $circuitDir
